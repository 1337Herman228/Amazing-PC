...
const AuthInput = (
    ...
    const ShowPasswordButton = () =>{
        return(
            <button className='form-field__label-password-show-btn' onClick={() => setIsShow(!isShow) }>
                {isShow ?
                    <img
    ...
    return (
        <div className='form-field'>
            ...
            <span className={`form-field__label-text ${require && 'require'}`}>
                {labelText}
            </span>
        </label>
        <div className={`input-wrapper ${type === 'password' ? 'no-mark' : errors[name]?.message ? 'error' : ''}`}>
            {isPhone == false &&
            <>
                <input 
