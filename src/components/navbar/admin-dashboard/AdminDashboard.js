...
const AdminDashboard = ({type}) => {
    ...
    const markCurrentLink = (currentUrl) => {
        ...
        for(var i=0;i<links.length;i++) {
            ...
            if(links[i].getAttribute('href').includes('/' +currentUrl.split('/')[1] + '/' + currentUrl.split('/')[2] + '/' + currentUrl.split('/')[3] )) {
        ...
        <header className='admin-dashboard-header'>
            <div className='admin-dashboard container'>
                <ul className='dashboard-list'>
                    {links.map((link)=>
                        <Link className='dashboard-list__link dashboard-link-to-check' href={link.link} key={link.id}>{link.title}</Link>
                    )}
                </ul>
            </div>
        </header>
