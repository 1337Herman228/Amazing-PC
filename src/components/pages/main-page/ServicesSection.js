...
const ServicesSection = () => {
     ...
     const accordeonClickFunction = (item) => {
          const list = document.querySelectorAll('.services__list__item__accordeon');
                    ...
                    <ul className='services__list'>
                         {list_info.map((item,i) =>
                              <li key={i} className='services__list__item'>
                                   <details className="services__list__item__accordeon" onClick={() => accordeonClickFunction(item)}>
                                        <summary className="services__list__item__accordeon__summary">
                                             <h3 className="services__list__item__accordeon__summary__title">{item.title}</h3>
                                             <svg className="services__list__item__accordeon__summary__indicator" width="34" height="34" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                  <path fillRule="evenodd" clipRule="evenodd" d="M12 4C12.5523 4 13 4.44772 13 5V11H19C19.5523 11 20 11.4477 20 12C20 12.5523 19.5523 13 19 13H13V19C13 19.5523 12.5523 20 12 20C11.4477 20 11 19.5523 11 19V13H5C4.44772 13 4 12.5523 4 12C4 11.4477 4.44772 11 5 11H11V5C11 4.44772 11.4477 4 12 4Z" fill="#c0ff01"/>
                                             </svg>
                                        </summary>
                                   ...
                                   <div className='services-what-we-can-do visible-tablet'>
                                        <div className='services-what-we-can-do__body'>
                                             <div className='services-what-we-can-do__body__title'>Что можно сделать</div>
                                             <ul className='services-what-we-can-do__body__list'>
                                                  {servListObj.services_list.map((item,i) =>
                                                       <li key={i} className='services-what-we-can-do__body__list__item'>
                                                            <img className="services-what-we-can-do__body__list__item__svg" width={50} height={50} src={item.list_item_svg_link} alt=''/>
                                                            <div className="services-what-we-can-do__body__list__item__description">{item.list_item_name}</div>
                                                       </li>
                                                  )}
                                             </ul>
                                             <Link className="services-what-we-can-do__body__link button-transparent-white" href={servListObj.link_to}>
               ...
               <div className='services-what-we-can-do hidden-tablet'>
                    <div className='services-what-we-can-do__body'>
                         ...
                         <ul className='services-what-we-can-do__body__list'>
                              {servListObj.services_list.map((item,i) =>
                                   <li key={i} className='services-what-we-can-do__body__list__item'>
                                        <img className="services-what-we-can-do__body__list__item__svg" width={70} height={70} src={item.list_item_svg_link} alt=''/>
                                        <div className="services-what-we-can-do__body__list__item__description">{item.list_item_name}</div>
                                   </li>
                              )}
                         </ul>
