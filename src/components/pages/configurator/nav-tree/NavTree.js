...
const NavTree = ({uniqueCategories, allItems}) => {
     ...
     useEffect(() => {
      ...
      const dynamicSecondNavLinks = () => {
          var navbarLinks = document.querySelectorAll(".tree-list__nav-list-item");
          // var mobileNavbarLinks = document.querySelectorAll(".second-mobile-navbar__item");

          var blocks = document.querySelectorAll(".managed-component-item")
     ...
     const onCategoryClick = (e) =>{
          ...
          setTimeout(() => {
               navList.classList.contains('open') ? null : navList.style.paddingBlock='0px';
          },250)
     }
     ...
     return(
          <>
               <ul className='tree-list'>
                    ...
                    <li key={index} className='tree-list__item'>
                         <button onClick={(e)=>onCategoryClick(e)} className='tree-list__item-open-btn active'>
                              {item}
                         </button>
                         <ul className='tree-list__nav-list open'>
                              {allItems.filter(el => el.category === item).map((item, index) =>
                              <>
                              ...
                              <Link key={index} href={`#${item.name}`} className='tree-list__nav-list-item'>
                                   <img
                                        ...
                                        loading='lazy'
                                   />
                                   <span className='tree-list__nav-list-item-name'>{item.name}</span>
                              </Link>
                              </>
