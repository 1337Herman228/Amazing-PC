                ...
                <table className='modal-table'>
                    <thead>
                        <tr className='modal-table__header'>
                            <th className='modal-table__header-text' colSpan={2}>Характеристики {name}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {makeObjectEntries(data).map(([key, value]) => (
                            <tr key={key} className='modal-table__row'>
                                <td className='modal-table__row-info'>{key}</td>
                                <td className='modal-table__row-price'>{value}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
